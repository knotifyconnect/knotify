...
import { Skeleton } from '@/components/ui/skeleton-1'
...
export const Avatar = ({ placeholder = false, size = 24, src }: AvatarProps) => {
  if (placeholder) {
    return <Skeleton rounded height={size} width={size} className="border border-white/20" />
  }

  return (
    <span className="inline-block overflow-hidden rounded-full border border-white/20 duration-200" style={{ width: size, height: size }}>
...
const GitHubIcon = () => (
  <svg className="h-3.5 w-3.5 text-[#111]" height="16" strokeLinejoin="round" viewBox="0 0 16 16" width="16">
...
function GitBadge({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={clsx('absolute -bottom-[5px] -left-[3px] flex items-center justify-center overflow-hidden rounded-full border border-[#0a0a0f]', className)}>
      {children}
    </div>
  )
}
...
export const GitHubAvatar = ({ username, size = 24, src }: GitAvatarProps) => {
  return (
    <div className="relative" style={{ width: size, height: size }}>
      <span className="inline-block overflow-hidden rounded-full border border-white/20 duration-200" style={{ width: size, height: size }}>
        {src ? <img src={src} alt={`Avatar for ${username}`} className="h-full w-full object-cover" /> : null}
      </span>
      <GitBadge className="bg-white">
        <GitHubIcon />
      </GitBadge>
    </div>
  )
...
export const AvatarGroup = ({ members, size = 24, limit = 3 }: AvatarGroupProps) => {
  const shown = members.length >= limit ? members.slice(0, limit - 1) : members
    ...
    <div className="flex items-center">
      {shown.map((member, index) => (
        <span
          ...
          aria-label={`Avatar for ${member.username ?? 'member'}`}
        >
          <span className="inline-block overflow-hidden rounded-full border border-white/20 duration-200" style={{ width: size, height: size }}>
            {member.src ? <img src={member.src} alt={`Avatar for ${member.username ?? 'member'}`} className="h-full w-full object-cover" /> : null}
          </span>
        </span>
      ))}

      {members.length === limit ? (
        <span className="-ml-2 inline-flex items-center" style={{ zIndex: limit }} aria-label="1 more avatars in this group">
          <span className="inline-block overflow-hidden rounded-full border border-white/20 duration-200" style={{ width: size, height: size }}>
            <img src="https://vercel.com/api/www/avatar?u=rauchg&s=64" alt="1 more avatars in this group" className="h-full w-full object-cover" />
          </span>
        </span>
      ) : null}
      ...
      {members.length > limit ? (
        ...
        >
          <span
            className="flex items-center justify-center overflow-hidden rounded-full border border-white/25 bg-white/80 text-[0.625rem] font-semibold leading-3 text-[#111] duration-200"
