...
conversationsRouter.get('/', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const myParticipantByConversationId = new Map<string, ParticipantRow>()
    for (const row of participantRows) {
      if (row.user_id === req.appUserId) {
        myParticipantByConversationId.set(row.conversation_id, row)
      }
    }
    ...
    const payload = ((conversations.data ?? []) as ConversationRow[]).map((conversation) => {
      ...
      return {
        ...
        cleared_at: myParticipantByConversationId.get(conversation.id)?.cleared_at ?? null,
